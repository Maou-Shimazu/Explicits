...
using Random = effolkronium::
    random_static;  // Documentation examples are listed here:
                    // https://github.com/effolkronium/random#five-minute-tutorial

/**
 * ---
 * Info: The general idea.
 * Store player/monster frequently updated configurations in either structs or
 * classes. Store other information like powers and landscape with values that
 * are only changed after certain events.
 * ---
 */
...
class Player {
 ...
 public:
  ...
  void power_iterator() {
    for (auto i : powers) std::cout << i.first + ", ";
  }
  void power_up();
  // Player attack prototype
  void attack();  // function prototype implimented below monster class creation
                  // because it wont be found in this scope.

  // Player heal function
  void heal() { health_value += Random::get<uint32_t>(16, 20); };
  // Player stats function
  void stats(std::string n = "") {
    std::cout << "Name: " << name << n << "Health: " << health_value << n
              << "Attack: " << attack_value << n << "Heal: " << heal_value << n
              << "Powers: ";
    power_iterator();
    std::cout << "\nMap: "
              << "\n";
...
class Monster : public Player {
 protected:
  ...
  std::vector<std::string> names_list = {
      "Marx", "Bambino", "Kale"};  // list of monster names that will be used
                                   // for monsters per level.
 ...
 public:
  ...
  void stats(std::string n = "") {
    std::cout << "Name: " << this->name << n << "Health: " << this->health_value
              << n << "Attack: " << this->attack_value << "\n";
  }
};
Monster monster;
...
void Player::power_up() {
  ...
  std::cout << "You activated: " << p << "! You dealt " << powers[p]
